...
import { CAPABILITY_OPTIONS } from '../data/assessmentData';
...
export const StepCapability: React.FC<StepCapabilityProps> = ({
  selectedId,
  onSelect,
  onNext,
  onBack
    ...
    <div className="max-w-3xl mx-auto py-8 sm:py-12 px-4">
      ...
      <div className="text-center mb-3">
        <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.25em] text-[#C5A059] font-bold">
          Stage 05 • Capability Priority
        </span>
      </div>

      {/* Main Question */}
      <h1 className="text-2xl sm:text-3xl text-center font-serif text-[#F5F5F0] font-light tracking-tight leading-tight mb-3">
        Where do your managers need the most development right now?
      </h1>

      <p className="text-center text-xs sm:text-sm text-white/50 max-w-lg mx-auto mb-10 font-light leading-relaxed">
        Identify the management capability area that would unlock the highest leverage for your organization.
      </p>

      {/* Selectable Options in 2 Columns on sm/md */}
        ...
        {CAPABILITY_OPTIONS.map((option) => {
          ...
          return (
            <div
              ...
              className={`group relative p-4.5 rounded-md border transition-all duration-200 cursor-pointer flex items-start gap-3.5 ${
                isSelected
            ...
            >
              ...
              <div className="flex-1">
                ...
                {option.description && (
                  <div className="text-xs text-slate-300/70 mt-1 font-light leading-relaxed">
                    {option.description}
                  </div>
                )}
              </div>
